...
**Last change pass:** 2026-09-13 against commit `4621fc8` (branch `feat/task-edit-history`)
plus the uncommitted working-tree change adding **task history** (due-date, owner and
status changes recorded in the task thread), the existing-task account gate, the creation
notification target, the previous-owner notification and Reopen in the account sidebar —
covering that change only. The previous passes were 2026-09-13 against `7c2e39f` plus the
working tree that became `12b7ce7` (account tasks editable), 2026-09-10 against `22d85a8`,
2026-09-09 against `d45a6cd` and 2026-07-31 against `15329e3`.
...
|---|---|---|---|---|---|---|---|
| Today | `/today` | [product/today](product/today/README.md) | Partially verified | 2026-07-31 | `4214349` | Priority ranking function not read end to end; commitments are mock-backed. One Permissions bullet (changing an existing task) amended 2026-09-13 at `4621fc8` + working tree; nothing else re-read | Unassigned |
...
| Client Profile → Account Tasks | `/clients/[id]` → Tasks button | [product/client-profile/account-tasks](product/client-profile/account-tasks.md) | Partially verified | 2026-09-13 | `4621fc8` + working-tree change | Tested: `lib/task-due.ts` (6) and `lib/task-activity.ts` (9) — due-date arithmetic and which changes produce history. Repository writers checked by hand against the test database, not repo-reproducible. Server actions, the account and owner gates, the transactional write, notifications and the components are untested and were not run against a signed-in session. Task history section added; comments in the thread (`TaskUpdates`) are referenced, not documented. Title/notes/priority/focus-area edits still leave no history; Guests cannot read threads (differs from spec `FR-011`) | Unassigned |
...
| Integrations & sync | `/settings?tab=integrations` | [product/integrations](product/integrations/README.md) | Partially verified | 2026-07-31 | `4214349` | Per-integration field mapping not documented. HubSpot account-discovery section re-verified 2026-09-21 against uncommitted changes on `580e0b7`; rest of the document not re-verified | Unassigned |
...
| [known-limitations](known-limitations/README.md) | Partially verified | 2026-09-09 | `15329e3` | Carries outstanding-work and never-browser-tested sections. *Task edits leave no history* added to Auditability 2026-09-13 at `7c2e39f` + working-tree change, and rewritten the same day as *Most task edits leave no history* at `4621fc8` + working-tree change; nothing else re-read. The `scratch-*` row was re-counted at `d45a6cd` (26 tracked routes, 8 unguarded in production); the Testing section's "seven test files, 124 tests" is **stale** — 24 test files exist |
| [contradictions](known-limitations/contradictions.md) | Verified | 2026-07-31 | `15329e3` | — |
| [releases/CHANGELOG.md](releases/CHANGELOG.md) | Partially verified | 2026-09-13 | `4621fc8` + working-tree change | Covers 2026-07-26 to 2026-07-31 plus the 2026-09-09 and 2026-09-10 Tech stack entries and the two 2026-09-13 account tasks entries (editing; task history). **2026-08-01 → 2026-09-08 is a gap** — roughly 40 commits with no entry. Backfill run figures are reported from an execution, not repo-verifiable |
...
| Task assignment and ownership | [permissions-and-scoping R6a](business-rules/permissions-and-scoping.md#r6a--assigning-a-task-to-someone-else-is-admin-only-and-refused-rather-than-downgraded) | Partially verified | 2026-09-13 | `4621fc8` + working-tree change | Documented as a rule within permissions. Self-reassignment and sidebar control gating added at `7c2e39f` + working tree; both-owner notification, the existing-task account gate (a tightening) and non-removable history added at `4621fc8` + working tree. No tests of the gates. `today_tasks` priority semantics still `Missing` |
